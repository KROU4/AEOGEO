...
2. **Settings**:
   ...
   - **Dockerfile Path**: `packages/web/Dockerfile.railway` (в репозитории добавлен специально под Railway: статика + `serve` на переменной **`PORT`**).
...
- Команда миграций: `uv run alembic upgrade head` из каталога приложения (`/work/packages/api` в нашем Dockerfile). В репозитории это задано в **`railway.toml`** → `[deploy].preDeployCommand`.
...
2. Временно **уберите** `preDeployCommand` из dashboard / закомментируйте в `railway.toml`, задеплойтесь, миграции — **один раз** из Shell.
