...
Сборка в образе — **`bun run build`** (только **Vite**, без отдельного пакета widget). Отдельный полный **`tsc`** в этот путь не входит (иначе на слабом builder CPU уходят десятки минут). Проверку типов держите в **CI** или локально: `cd packages/web && bun run typecheck`. Строгая сборка с `tsc` перед Vite: `bun run build:strict`.
